...
import { Phone, PhoneOff, Loader2, CheckCircle, Plus, ArrowRight, MessageSquare, Bot, Users } from 'lucide-react';
import VoiceAvatar from './components/VoiceAvatar';
import { clientConfig } from '@/config/client';
import { createClient } from '@/lib/supabase';

type SetupState = 'loading' | 'dashboard' | 'ready_for_setup' | 'setup_in_progress' | 'processing' | 'panel_ready' | 'error';

interface Panel { id: string; name: string; description: string; created_at: string; }
...
export default function SetupClient() {
  ...
  async function loadPanels() {
    try {
      ...
      const { data } = await supabase.from('agents').select('id, name, description, created_at').order('created_at', { ascending: false }) as { data: Panel[] | null };
      setPanels(data || []);
      setState(data && data.length > 0 ? 'dashboard' : 'ready_for_setup');
    } catch (err) { console.error('Failed to load panels:', err); setState('ready_for_setup'); }
  }

  const startSetupCall = async () => { widgetMountedRef.current = false; setState('setup_in_progress'); setShowWidget(true); };
  ...
  const endCall = async () => {
    ...
    const checkForNewPanel = async () => {
      const supabase = createClient();
      const { data } = await supabase.from('agents').select('id, name').order('created_at', { ascending: false }).limit(1) as { data: { id: string; name: string }[] | null };
      if (data && data.length > 0) {
        const latestPanel = data[0];
        if (!panels.find(p => p.id === latestPanel.id)) { setCurrentPanelId(latestPanel.id); setState('panel_ready'); return; }
      }
  ...
  const WidgetContainer = () => (<div ref={widgetContainerRef} className={\`mb-6 min-h-[80px] \${showWidget ? 'block' : 'hidden'}\`} />);

  const renderContent = () => {
    switch (state) {
      case 'loading': return (<div className="text-center"><Loader2 className="w-12 h-12 text-purple-500 animate-spin mx-auto mb-4" /><p className="text-slate-400">Loading...</p></div>);
      case 'dashboard': return (<div className="max-w-4xl mx-auto w-full"><div className="flex items-center justify-between mb-8"><div><h1 className="text-2xl font-bold">{clientConfig.platform.name}</h1><p className="text-slate-400">{clientConfig.company.name}</p></div><button onClick={() => setState('ready_for_setup')} className="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded-lg flex items-center gap-2 transition"><Plus className="w-4 h-4" />New Panel</button></div><div className="grid gap-4">{panels.map((panel) => (<div key={panel.id} className="bg-slate-900 rounded-xl p-4 flex items-center justify-between"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-purple-500/20 rounded-full flex items-center justify-center"><Bot className="w-6 h-6 text-purple-400" /></div><div><h3 className="font-medium">{panel.name}</h3><p className="text-sm text-slate-400">{panel.description || 'No description'}</p></div></div><div className="flex items-center gap-2"><a href={\`/i/\${panel.id}\`} target="_blank" className="bg-slate-800 hover:bg-slate-700 px-3 py-2 rounded-lg flex items-center gap-2 text-sm transition"><MessageSquare className="w-4 h-4" />Test</a><a href={\`/panel/\${panel.id}/invite\`} className="bg-purple-600 hover:bg-purple-500 px-3 py-2 rounded-lg flex items-center gap-2 text-sm transition"><Users className="w-4 h-4" />Invite</a></div></div>))}</div></div>);
      case 'ready_for_setup': return (<div className="text-center max-w-lg mx-auto"><VoiceAvatar size="lg" label="AI Setup Assistant" /><h1 className="text-3xl font-bold mb-4">{panels.length > 0 ? 'Create New Panel' : 'Welcome!'}</h1><p className="text-slate-300 mb-2">{panels.length > 0 ? "Let's set up another interview panel." : "Let's create your first AI interview panel."}</p><p className="text-slate-400 mb-8">Tell the assistant what kind of interviews or surveys you want to run.</p><button onClick={startSetupCall} disabled={!SETUP_AGENT_ID || !scriptLoaded} className="inline-flex items-center gap-3 bg-green-600 hover:bg-green-500 px-8 py-4 rounded-xl font-semibold text-lg transition-all hover:scale-105 shadow-lg shadow-green-500/25 disabled:opacity-50"><Phone className="w-6 h-6" />{scriptLoaded ? 'Start Setup Call' : 'Loading...'}</button>{panels.length > 0 && (<button onClick={() => setState('dashboard')} className="block mx-auto mt-4 text-slate-400 hover:text-white transition">Back to Dashboard</button>)}<WidgetContainer /></div>);
      case 'setup_in_progress': return (<div className="text-center max-w-lg mx-auto"><VoiceAvatar isActive isSpeaking size="lg" label="Speaking..." /><h2 className="text-2xl font-bold text-green-400 mb-4">Call in Progress</h2><p className="text-slate-400 mb-6">Describe your interview panel. When finished, click End Call.</p><WidgetContainer /><button onClick={endCall} className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-500 px-6 py-3 rounded-lg font-medium transition"><PhoneOff className="w-5 h-5" />End Call</button></div>);
      case 'processing': return (<div className="text-center max-w-lg mx-auto"><VoiceAvatar isActive size="lg" label="Creating panel..." /><h2 className="text-2xl font-bold mb-4">Creating Your Panel</h2><p className="text-slate-400 mb-8">Processing your requirements...</p><div className="space-y-3 text-left bg-slate-900 rounded-xl p-6"><div className="flex items-center gap-3"><CheckCircle className="w-5 h-5 text-green-500" /><span className="text-slate-300">Setup conversation captured</span></div><div className="flex items-center gap-3"><Loader2 className="w-5 h-5 text-purple-400 animate-spin" /><span className="text-purple-400">Building interview agent...</span></div></div></div>);
      case 'panel_ready': return (<div className="text-center max-w-lg mx-auto"><VoiceAvatar size="lg" label="Panel ready!" /><h2 className="text-3xl font-bold text-green-400 mb-4">Panel Created!</h2><p className="text-slate-400 mb-8">Your interview panel is ready. You can now invite interviewees.</p><div className="flex flex-col gap-3"><a href={\`/panel/\${currentPanelId}/complete\`} className="inline-flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-500 px-6 py-3 rounded-lg font-semibold transition">View Panel & Invite<ArrowRight className="w-5 h-5" /></a><button onClick={() => { loadPanels(); setState('dashboard'); }} className="text-slate-400 hover:text-white transition">Go to Dashboard</button></div></div>);
      case 'error': return (<div className="text-center max-w-lg mx-auto"><h2 className="text-2xl font-bold text-red-400 mb-4">Something Went Wrong</h2><p className="text-slate-400 mb-8">{error}</p><button onClick={() => setState('ready_for_setup')} className="bg-purple-600 hover:bg-purple-500 px-6 py-3 rounded-lg">Try Again</button></div>);
      default: return null;
    }
  };

  return (<div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">{renderContent()}</div>);
}`,
...
interface VoiceAvatarProps { isActive?: boolean; isSpeaking?: boolean; size?: 'sm' | 'md' | 'lg'; label?: string; }
  ...
  return (
    <div className="flex flex-col items-center mb-6">
      <div className="relative">
        {isActive && (<><div className={\`absolute inset-0 \${sizeClasses[size]} rounded-full bg-purple-500/20 animate-ping\`} /><div className={\`absolute inset-0 \${sizeClasses[size]} rounded-full bg-purple-500/10 animate-pulse\`} /></>)}
        {isSpeaking && <div className={\`absolute inset-0 \${sizeClasses[size]} rounded-full border-4 border-green-400 animate-pulse\`} />}
        <div className={\`relative \${sizeClasses[size]} rounded-full flex items-center justify-center \${isActive ? 'bg-gradient-to-br from-purple-600 to-purple-800' : 'bg-slate-800'} shadow-xl\`}><Bot className={\`\${iconSizes[size]} \${isActive ? 'text-white' : 'text-purple-400'}\`} /></div>
      </div>
...
export default function PanelCompletePage() {
  ...
  useEffect(() => { if (typeof window !== 'undefined') setInterviewUrl(\`\${window.location.origin}/i/\${panelId}\`); loadPanel(); }, [panelId]);

  async function loadPanel() {
    try { const supabase = createClient(); const { data } = await supabase.from('agents').select('*').eq('id', panelId).single(); setPanel(data); }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="text-center mb-10"><div className="w-20 h-20 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-6"><CheckCircle className="w-10 h-10 text-green-400" /></div><h1 className="text-3xl font-bold mb-2">Great Work!</h1><p className="text-xl text-slate-300">Your <span className="text-purple-400">{panel.name}</span> panel is ready</p></div>
        <div className="bg-slate-900 rounded-2xl p-6 mb-6"><label className="block text-sm text-slate-400 mb-2">Interview Link</label><div className="flex gap-2"><input type="text" value={interviewUrl} readOnly className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-sm" /><button onClick={copyLink} className={\`px-4 py-2 rounded-lg flex items-center gap-2 transition \${copied ? 'bg-green-600' : 'bg-purple-600 hover:bg-purple-500'}\`}><Copy className="w-4 h-4" />{copied ? 'Copied!' : 'Copy'}</button></div></div>
        <div className="grid sm:grid-cols-2 gap-4 mb-8"><Link href={\`/i/\${panelId}\`} className="flex items-center justify-center gap-3 bg-green-600 hover:bg-green-500 px-6 py-4 rounded-xl font-semibold transition"><Phone className="w-5 h-5" />Test Interview</Link><Link href={\`/panel/\${panelId}/invite\`} className="flex items-center justify-center gap-3 bg-purple-600 hover:bg-purple-500 px-6 py-4 rounded-xl font-semibold transition"><Users className="w-5 h-5" />Invite Interviewees</Link></div>
...
import { useParams } from 'next/navigation';
import Link from 'next/link';
import { Upload, Send, Loader2, CheckCircle, AlertCircle, Download, ArrowLeft, FileSpreadsheet, X, User } from 'lucide-react';
import { createClient } from '@/lib/supabase';
...
export default function InvitePage() {
  ...
  async function loadPanel() {
    try { const supabase = createClient(); const { data } = await supabase.from('agents').select('id, name').eq('id', panelId).single(); setPanel(data); }
    catch (err) { console.error('Failed to load panel:', err); }
    finally { setLoading(false); }
  }

  async function handleSingleInvite(e: React.FormEvent) {
    e.preventDefault(); setSendingSingle(true); setSingleResult(null);
    try {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0]; if (!file) return; setParseError(''); setBulkInterviewees([]);
    try {
  ...
  async function handleBulkInvite() {
    if (bulkInterviewees.length === 0) return; setSendingBulk(true); setBulkResults([]);
    try {
  ...
  function downloadTemplate() { const csv = 'name,email,custom_field\\nJohn Doe,john@example.com,Sales Team\\nJane Smith,jane@example.com,Marketing'; const blob = new Blob([csv], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'interviewees_template.csv'; a.click(); URL.revokeObjectURL(url); }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8"><Link href={\`/panel/\${panelId}/complete\`} className="text-slate-400 hover:text-white flex items-center gap-2 mb-4"><ArrowLeft className="w-4 h-4" />Back to Panel</Link><h1 className="text-2xl font-bold">Invite Interviewees</h1><p className="text-slate-400">{panel?.name}</p></div>
        <div className="flex gap-2 mb-6"><button onClick={() => setActiveTab('single')} className={\`px-4 py-2 rounded-lg flex items-center gap-2 transition \${activeTab === 'single' ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}\`}><User className="w-4 h-4" />Single Invite</button><button onClick={() => setActiveTab('bulk')} className={\`px-4 py-2 rounded-lg flex items-center gap-2 transition \${activeTab === 'bulk' ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}\`}><FileSpreadsheet className="w-4 h-4" />Bulk Upload</button></div>
        {activeTab === 'single' && (<div className="bg-slate-900 rounded-2xl p-6"><form onSubmit={handleSingleInvite} className="space-y-4"><div><label className="block text-sm text-slate-400 mb-1">Name</label><input type="text" value={singleName} onChange={(e) => setSingleName(e.target.value)} placeholder="John Doe" className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 focus:outline-none focus:border-purple-500" required /></div><div><label className="block text-sm text-slate-400 mb-1">Email</label><input type="email" value={singleEmail} onChange={(e) => setSingleEmail(e.target.value)} placeholder="john@example.com" className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 focus:outline-none focus:border-purple-500" required /></div>{singleResult && <div className={\`p-3 rounded-lg flex items-center gap-2 \${singleResult.success ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}\`}>{singleResult.success ? <CheckCircle className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}{singleResult.success ? 'Invite sent successfully!' : singleResult.error}</div>}<button type="submit" disabled={sendingSingle} className="w-full bg-purple-600 hover:bg-purple-500 px-4 py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition disabled:opacity-50">{sendingSingle ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}Send Invite</button></form></div>)}
        {activeTab === 'bulk' && (<div className="space-y-6"><div className="bg-slate-900 rounded-2xl p-6"><div className="flex items-center justify-between mb-4"><h3 className="font-medium">Upload Excel or CSV</h3><button onClick={downloadTemplate} className="text-sm text-purple-400 hover:text-purple-300 flex items-center gap-1"><Download className="w-4 h-4" />Download Template</button></div><input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} className="hidden" /><button onClick={() => fileInputRef.current?.click()} className="w-full border-2 border-dashed border-slate-700 hover:border-purple-500 rounded-xl p-8 flex flex-col items-center gap-2 transition"><Upload className="w-8 h-8 text-slate-400" /><span className="text-slate-400">Click to upload .xlsx, .xls, or .csv</span><span className="text-xs text-slate-500">Required columns: name, email</span></button>{parseError && <div className="mt-4 p-3 bg-red-500/20 text-red-400 rounded-lg flex items-center gap-2"><AlertCircle className="w-4 h-4" />{parseError}</div>}</div>{bulkInterviewees.length > 0 && (<div className="bg-slate-900 rounded-2xl p-6"><div className="flex items-center justify-between mb-4"><h3 className="font-medium">{bulkInterviewees.length} interviewees ready</h3><button onClick={() => setBulkInterviewees([])} className="text-sm text-slate-400 hover:text-red-400">Clear All</button></div><div className="max-h-64 overflow-y-auto space-y-2 mb-4">{bulkInterviewees.map((person, i) => (<div key={i} className="flex items-center justify-between bg-slate-800 rounded-lg px-3 py-2"><div><span className="font-medium">{person.name}</span><span className="text-slate-400 ml-2">{person.email}</span></div><button onClick={() => setBulkInterviewees(bulkInterviewees.filter((_, idx) => idx !== i))} className="text-slate-400 hover:text-red-400"><X className="w-4 h-4" /></button></div>))}</div><button onClick={handleBulkInvite} disabled={sendingBulk} className="w-full bg-purple-600 hover:bg-purple-500 px-4 py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition disabled:opacity-50">{sendingBulk ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}Send All Invites</button></div>)}{bulkResults.length > 0 && (<div className="bg-slate-900 rounded-2xl p-6"><h3 className="font-medium mb-4">Results</h3><div className="space-y-2">{bulkResults.map((result, i) => (<div key={i} className={\`flex items-center gap-2 p-2 rounded-lg \${result.success ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}\`}>{result.success ? <CheckCircle className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}<span>{result.email}</span>{!result.success && <span className="text-xs">- {result.error}</span>}</div>))}</div></div>)}</div>)}
      </div>
    </div>
  );
}`,
...
export default function InterviewPage() {
  ...
  async function loadAgent() {
    try { const res = await fetch(\`/api/agents/\${agentId}\`); if (!res.ok) throw new Error('Agent not found'); setAgent(await res.json()); }
  ...
  async function validateToken() {
    ...
    try { const res = await fetch(\`/api/invites/validate?token=\${inviteToken}\`); if (res.ok) { const data = await res.json(); setIntervieweeId(data.intervieweeId); } }
  ...
  async function startInterview() {
    ...
    const script = document.createElement('script'); script.src = 'https://elevenlabs.io/convai-widget/index.js'; script.async = true;
    script.onload = () => { const container = document.getElementById('widget-container'); if (container && agent?.elevenlabs_agent_id) { container.innerHTML = \`<elevenlabs-convai agent-id="\${agent.elevenlabs_agent_id}"></elevenlabs-convai>\`; setStatus('active'); } };
    document.body.appendChild(script);
  }

  async function endInterview() {
    const container = document.getElementById('widget-container'); if (container) container.innerHTML = '';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full text-center">
        <div className="mb-8"><div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: (agent.primary_color || '#8B5CF6') + '20' }}><Bot className="w-10 h-10" style={{ color: agent.primary_color || '#8B5CF6' }} /></div><h1 className="text-2xl font-bold">{agent.name}</h1><p className="text-slate-400">{agent.company_name}</p></div>
